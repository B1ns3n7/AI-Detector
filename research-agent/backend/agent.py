...
from tools.doaj_tool import search_doaj, get_doaj_article
...
def _collect_papers(query: str, max_papers: int,
    ...
    all_search_fns = [
        ...
        ("DOAJ",                   lambda q, o=offset: search_doaj(q, offset=o)),
...
def _format_papers_as_context(papers: list) -> str:
    """
    Full paper context for follow-up Q&A and RRL generation.
    Includes all APA 7 required fields: authors, year, title,
    ...
    for i, p in enumerate(papers):
        ...
        # Full author list for APA 7 (up to 20 authors)
        authors = p.get("authors", [])
        if authors:
            lines.append(f"Authors: {'; '.join(authors[:20])}")
            lines.append(f"Author count: {len(authors)}")
        ...
        if p.get("citations") is not None:
            lines.append(f"Citations: {p['citations']}")
        if p.get("source"):
            lines.append(f"Source database: {p['source']}")

        # DOI — most important for APA 7
        if p.get("doi"):
            doi = p["doi"].lstrip("https://doi.org/").lstrip("doi:")
            lines.append(f"DOI: https://doi.org/{doi}")

        # URL fallback if no DOI
        if not p.get("doi") and p.get("url"):
            lines.append(f"URL: {p['url']}")
        if not p.get("doi") and p.get("pdf_url"):
            lines.append(f"PDF URL: {p['pdf_url']}")

        # PubMed ID for PubMed papers
        if p.get("pmid"):
            lines.append(f"PubMed ID: {p['pmid']}")
    ...
    async def run_research(self, query: str, max_papers: int = 5, mode: str = "full",
        ...
        try:
            ...
            if page > 0 and session_id in self.sessions:
                ...
                # Regenerate summary using ALL accumulated papers
                all_papers = existing["papers"]
                updated_summary = self._generate_summary(
                    existing["query"], all_papers, existing.get("mode", "full")
                )
                existing["summary"] = updated_summary
            ...
            # but include the updated summary covering ALL papers.
            # Session stores the full cumulative list for follow-up Q&A.
            ...
            response["summary"]       = self.sessions[session_id]["summary"]  # updated
            response["updated_summary"] = True   # flag so frontend knows to update
            return response
        except Exception as e:
