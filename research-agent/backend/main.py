...
async def rrl_stream(query: FollowUpQuery):
    ...
    def generate_stream():
        ...
        if gemini_key and gemini_key != "your_gemini_api_key_here":
            try:
                gemini_notice = json.dumps({'text': '⏳ Groq busy — using Gemini...\n\n'})
                yield f"data: {gemini_notice}\n\n"
                contents = []
                sys_text = ""
                for m in messages:
                    if m["role"] == "system":
                ...
                err_msg = json.dumps({'text': f'Error: {str(e)}'})
                yield f"data: {err_msg}\n\n"

        no_llm_msg = json.dumps({'text': 'Error: No LLM available. Check API keys.'})
        yield f"data: {no_llm_msg}\n\n"
        yield "data: [DONE]\n\n"
