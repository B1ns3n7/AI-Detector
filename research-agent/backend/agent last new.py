...
def _collect_papers(query: str, max_papers: int,
                    ...
                    selected_sources: list = None) -> list:
    """
    ...
    if scopus_ok or user_wants_scopus:
        all_search_fns.append(("Scopus", lambda q: search_scopus(q)))
    if springer_ok or user_wants_springer:
        all_search_fns.append(("Springer Nature", lambda q: search_springer(q)))
...
class ResearchAgent:
    ...
    def _run_research(self, query: str, max_papers: int, mode: str, selected_sources: list = None) -> tuple:
        ...
        papers = _collect_papers(query, max_papers, scopus_ok, springer_ok, selected_sources)
    ...
    async def run_research(self, query: str, max_papers: int = 5, mode: str = "full", selected_sources: list = None) -> dict:
        session_id = str(uuid.uuid4())
        start_time = datetime.now()
        try:
            summary, papers, steps = await asyncio.get_event_loop().run_in_executor(
                None, lambda: self._run_research(query, max_papers, mode, selected_sources)
            )
            for paper in papers:
