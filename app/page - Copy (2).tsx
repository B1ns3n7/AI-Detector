...
async function generatePDFReport(
  ...
  sf("normal", 8, C.s400);
  tx("Perplexity & Stylometry  -  Burstiness & Cognitive Markers  -  Neural Perplexity  -  Comparative Analysis", ML, 26);
...
function runPerplexityEngine(text: string): EngineResult {
  ...
  const activeSignals = [vocabScore, transScore, bigramScore, ttrScore, nomScore, rhythmScore, structureScore, ethicsScore, tricolonScore, llamaScore, claudeCatchScore, paraOpenerScore, conclusionScore, syntaxScore]
    .filter(s => s > 5).length;

  const rawTotal = vocabScore + transScore + bigramScore + ttrScore + nomScore + rhythmScore + structureScore + ethicsScore + tricolonScore + llamaScore + claudeCatchScore + paraOpenerScore + conclusionScore + syntaxScore;
  const maxTotal = 35 + 35 + 30 + 25 + 20 + 20 + 25 + 20 + 20 + 28 + 24 + 30 + 22 + 28; // 362
  ...
  if (eslFlag) {
    ...
    const eslSafeRaw = structureScore + ethicsScore + tricolonScore;
    const eslSafeNorm = Math.min(100, (eslSafeRaw / maxTotal) * 100);
    // Blend: 95% weight on ESL-safe signals, 5% on full score (strong suppression of vocab/transition noise)
    // Raised from 80/20 → 95/5 to prevent ESL false positives when vocab signals are present
  ...
  const { low, high, strength, phrase } = computeConfidenceInterval(
    rawScore, 9, signalsAgreeing, reliabilityWarnings, wc
  );
...
function runBurstinessEngine(text: string): EngineResult {
  ...
  const rawTotal = effectiveBurstScore + noShortScore + effectiveRangeScore + effectiveRhetoricalScore + effectiveMinFloor - contrReduction;
  const maxTotal = 40 + 15 + 18 + 20 + 22; // 115 (contraction is a reduction)
  ...
  const activeSignals = [effectiveBurstScore > 10, noShortScore > 0, effectiveRangeScore > 5, effectiveRhetoricalScore > 8, effectiveMinFloor > 0].filter(Boolean).length;
  ...
  const { low, high, strength, phrase } = computeConfidenceInterval(
    rawScore, 5, activeSignals, reliabilityWarnings, wc
  );
...
function ParagraphHeatMap({ sentences, originalText }: { sentences: SentenceResult[]; originalText: string }) {
  ...
  // Split original text into paragraphs, map each paragraph to its sentences
  const rawParas = originalText.split(/\n\n+/).map(p => p.trim()).filter(p => p.length > 20);
  if (rawParas.length < 2) return null; // not worth showing for single-paragraph texts

  // Assign sentences to paragraphs greedily by text matching
  ...
  for (let pi = 0; pi < rawParas.length; pi++) {
    ...
    const paraSents: SentenceResult[] = [];
    // Grab sentences whose text appears in this paragraph
    while (sentIdx < sentences.length && para.includes(sentences[sentIdx].text.trim().slice(0, 30))) {
      paraSents.push(sentences[sentIdx]);
      sentIdx++;
    }
    // Fallback: if no match, assign next available sentence
    if (paraSents.length === 0 && sentIdx < sentences.length) {
      paraSents.push(sentences[sentIdx++]);
    }
    ...
    paraData.push({
      paraIdx: pi,
      text: para.slice(0, 80) + (para.length > 80 ? "…" : ""),
      sentences: paraSents,
          ...
          <div className="px-5 pt-4 pb-3 border-b border-slate-100 space-y-3">
            ...
            {result.reliabilityWarnings.length > 0 && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 space-y-0.5">
                ...
                {result.reliabilityWarnings.map((w, i) => (
...
async function runNeuralEngine(text: string): Promise<EngineResult> {
  const sentences = splitSentences(text);
...
Also provide:
- overall_score: weighted composite of the above (0-100, higher = more AI-like)
- evidence_strength: one of "INCONCLUSIVE", "LOW", "MEDIUM", "HIGH"
- verdict_phrase: a single concise sentence describing the result
- reliability_notes: array of strings noting any factors (ESL, academic register, short text, technical content) that reduce confidence
...
Return exactly this JSON shape:
{
  ...
  const USER_PROMPT = `Analyze this text:\n\n${analysisText}`;
  ...
  try {
    ...
    // Derive basic sentence-level estimates locally so Sentence-Level Pattern
    // Analysis is never empty even when the API fails.
    const fallbackSentences: SentenceResult[] = sentences.map(sent => {
      const sw = sent.toLowerCase().match(/\b[a-z]+\b/g) || [];
      const vocabHits = sw.filter((w: string) => AI_VOCAB.has(w)).length;
      let raw = vocabHits >= 3 ? 40 : vocabHits >= 1 ? 20 : 0;
      let sTrans = 0;
      AI_TRANSITIONS.forEach((p: RegExp) => { const m = sent.match(p); if (m) sTrans += m.length; });
      if (sTrans > 0) raw += Math.min(18, sTrans * 9);
      if (/\b(yeah|yep|nope|gonna|wanna|kinda|dunno)\b/i.test(sent)) raw = Math.max(0, raw - 25);
      if (/\.{3}|…/.test(sent)) raw = Math.max(0, raw - 8);
      if (/\?/.test(sent)) raw = Math.max(0, raw - 12);
      if ((sent.match(TRUE_CONTRACTION_RE) || []).length > 0) raw = Math.max(0, raw - 10);
      const likelihood = Math.round(Math.min(85, Math.max(0, (raw / 60) * 100)));
      const label: "uncertain" | "moderate" | "elevated" =
        likelihood >= 50 ? "elevated" : likelihood >= 25 ? "moderate" : "uncertain";
      const sigs: string[] = [];
      if (vocabHits > 0) sigs.push(`${vocabHits} AI-associated word${vocabHits > 1 ? "s" : ""} (local estimate)`);
      if (sTrans > 0) sigs.push("AI transition phrase (local estimate)");
      return { text: sent, likelihood, signals: sigs, label };
    });
    // Derive a fallback internalScore from sentence-level evidence so that
    // AI% is never 0% when elevated sentences are present (even on API failure).
    const fbElevated = fallbackSentences.filter(s => s.label === "elevated").length;
    const fbModerate = fallbackSentences.filter(s => s.label === "moderate").length;
    const fbTotal    = Math.max(fallbackSentences.length, 1);
    const fbScore    = Math.min(20, Math.round(((fbElevated * 2 + fbModerate) / fbTotal) * 15));
    return {
      internalScore: fbScore, confidenceLow: 0, confidenceHigh: 50,
      evidenceStrength: "INCONCLUSIVE" as EvidenceStrength,
      verdictPhrase: "Neural engine unavailable — sentence highlights are local estimates",
      signals: [{
        name: "Neural Perplexity",
        value: "API call failed. Sentence highlights below are derived from local heuristics only.",
        strength: 0, pointsToAI: false, wellSupported: false,
      }],
      sentences: fallbackSentences,
      wordCount: wc,
      sentenceCount: sentences.length,
      reliabilityWarnings: ["Neural engine API error — sentence patterns shown are local estimates only, not neural analysis"],
    };
  }
  ...
  const { low, high } = computeConfidenceInterval(score, 6, signals.filter(s => s.pointsToAI).length, parsed.reliability_notes ?? [], wc);
...
export default function DetectorPage() {
  ...
  const handleAnalyze = useCallback(() => {
    ...
    runNeuralEngine(trimmed).then(nResult => {
    ...
    {/*     <div className="bg-red-50 border border-red-200 rounded-2xl px-5 py-3 flex items-start gap-3">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          <EnginePanel
            ...
            logoText="PS"
            logoBg="bg-[#1b3a6b]"
            methodology="15 signals: vocab density, transition phrases, bigrams, paragraph openers, conclusion clustering, passive voice, syntactic uniformity + more."
          ...
          <EnginePanel
            ...
            logoText="BC"
            logoBg="bg-green-700"
            methodology="Strict CV thresholds: CV < 0.15 = strong AI, CV < 0.35 = moderate. Expanded pattern detection."
          ...
          <EnginePanel
            ...
            methodology="LLM-based analysis: token predictability, semantic smoothness, structural uniformity, human cognitive markers."
          ...
          <div className="bg-slate-50 text-slate-600 rounded-2xl p-5 text-xs leading-relaxed grid sm:grid-cols-3 gap-6 border border-slate-200">
            <div>
              ...
              <p className="text-slate-500">
                Detects AI vocabulary density, transition phrases, bigram patterns, paragraph-opening fingerprints, conclusion ethics clustering, passive voice & syntactic uniformity, and document-level structure — 15 signals total. Even a single strong signal raises the evidence level.
              </p>
            </div>
            <div>
              <p className="font-semibold text-slate-800 mb-1.5 flex items-center gap-2">
              ...
              <p className="text-slate-500">
                Measures sentence length variation (CV). Strict thresholds: CV &lt; 0.15 = strong AI signal, CV &lt; 0.35 = moderate AI signal. Rhetorical devices are counted as human signals. Score of 25%+ triggers Moderate AI verdict; 45%+ triggers High AI verdict regardless of engine disagreement.
		{/* (Turnitin/GPTZero aligned). */}
            ...
            <div>
              ...
              <p className="text-slate-500">
                Uses a language model to evaluate token-level predictability, semantic coherence, and structural fingerprints that rule-based engines cannot reason about. Improves accuracy on paraphrased AI text, ESL content, and mixed-authorship documents by applying contextual judgment beyond surface-level patterns.
              </p>
            </div>
          </div>
        )}
