...
function boostPerplexity(
  ...
  // Find a middle sentence that hasn't been modified yet
...
function reduceRepetition(
  ...
  let changed = 0;
  const MAX_CHANGES = 2; // cap to avoid over-fronting
  ...
  for (let i = 1; i < sentences.length - 1 && changed < MAX_CHANGES; i++) {
    ...
    if ((sameOpener || (bothIsPattern && intensity === "aggressive")) && !curr.startsWith("In ") && !curr.startsWith("From ")) {
      ...
      count++;
      changed++;
      i++; // skip next sentence to avoid back-to-back fronting
    }
  }
...
function addReadabilityVariation(
  ...
  // Insert a qualifier after a verb in one unmodified sentence
  // Guard: only insert when verb is mid-sentence and sentence is long enough
  for (let i = 0; i < sentences.length; i++) {
    if (modifiedIndices.has(i)) continue;
...
function humanizeText(
  ...
  // ── Step 1: Vocabulary replacement ───────────────────────────────────────
  // Replaces AI-flagged words/phrases with natural alternatives.
  // Uses regex with word boundaries — guaranteed safe, never breaks sentence structure.
  const s1 = replaceVocabulary(text, tone, intensity);
  if (s1.count > 0) {
    changes.push({
      type: "vocabulary",
      description: `Replaced ${s1.count} formulaic word or phrase — e.g. "furthermore" → "also", "utilize" → "use", "pivotal" → "key". Each substitution preserves meaning while reducing mechanical phrasing patterns common in AI-generated prose.`,
    });
  }

  // ── Step 2: Verbose construction simplification ───────────────────────────
  // Replaces multi-word AI constructions with shorter equivalents.
  // All replacements are strictly whole-phrase → shorter phrase; no word insertions.
  const s2 = targetDetectorSignals(s1.text, intensity);
  if (s2.count > 0) {
    changes.push({
      type: "transition",
      description: `Simplified ${s2.count} verbose construction${s2.count > 1 ? "s" : ""} — e.g. "in order to" → "to", "due to the fact that" → "because", "has the ability to" → "can". Reduces nominal bloat and improves reading fluency without altering meaning.`,
    });
  }

  // ── Step 3: Passive-to-active rewriting ──────────────────────────────────
  // Rewrites specific passive AI patterns to active voice.
  // Only fires on exact matches — safe because replacements are fully tested.
  const s3 = rewriteStructure(s2.text, tone, intensity);
  if (s3.count > 0) {
    changes.push({
      type: "restructure",
      description: `Restructured ${s3.count} sentence${s3.count > 1 ? "s" : ""} — rewrote passive constructions to active voice (e.g. "It is widely recognized that" → "Most people agree that"). Improves authorial directness and clarity per academic style standards. Tone-conditioned for ${tone} register.`,
    });
  }

  // ── Step 4: Contraction insertion ────────────────────────────────────────
  // Replaces two-word forms with contractions using word-boundary regex.
  // Academic tone skips this entirely. Professional skips most.
  const s4 = insertContractions(s3.text, tone, intensity);
  if (s4.count > 0) {
    changes.push({
      type: "tone",
      description: `Inserted ${s4.count} contraction${s4.count > 1 ? "s" : ""} — e.g. "do not" → "don't", "cannot" → "can't". Reduces the unnaturally formal register characteristic of AI prose. Skipped for Academic tone to preserve scholarly register.`,
    });
  }

  // ── Step 5: Burstiness — sentence splitting ───────────────────────────────
  // Splits long sentences at natural grammar points (commas before conjunctions).
  // Never inserts words — only moves a split point.
  const s5 = injectBurstiness(s4.text, intensity);
  if (s5.splits > 0) {
    changes.push({
      type: "burstiness",
      description: `Split ${s5.splits} long sentence${s5.splits > 1 ? "s" : ""} at natural conjunction points — ", and", ", but", "; ". Introduces sentence-length variation (burstiness) that characterises natural academic prose and distinguishes it from the uniform rhythm of AI-generated text.`,
    });
  }
  ...
  if (changes.length === 0) {
    changes.push({
      type: "vocabulary",
      description: "No strong mechanical patterns detected. Try Aggressive intensity for deeper structural refinement, or paste text that contains formulaic transitions, verbose constructions, or uniform sentence rhythm.",
    });
  }
...
export default function HumanizerPage() {
  ...
  const handleHumanize = useCallback(async () => {
    ...
    setResult(null);
    try {
      const intensityLabel = intensity === "subtle" ? "minimal" : intensity === "moderate" ? "moderate" : "thorough";
      const toneLabel = tone === "academic" ? "scholarly academic (no contractions)" : tone === "professional" ? "professional and formal" : tone === "neutral" ? "clear and neutral" : "natural and conversational";

      const systemPrompt = `You are a human writing coach editing a draft to sound like a real person wrote it — not an AI. Your job is to make the text feel genuinely human: direct, slightly imperfect, varied, and natural.

You must respond ONLY with a valid JSON object. No preamble, no markdown fences, no commentary outside the JSON.

STRICT RULES — violating any of these is a failure:

NEVER use these words or phrases (they are AI giveaways):
- commence, utilize, facilitate, underscore, notable, notably, it is worth noting, in the context of, in large measure, of considerable value, in a manner, whereby, thereby, thus, hence, therein, aforementioned, multifaceted, nuanced, robust, seamless, paradigm, tapestry, it is important to note, plays a role, serves to, functions to, operates to, permits, enables (when replacing simple verbs), in terms of, with regard to, with respect to, in order to, due to the fact that, has the ability to, at this point in time, in the event that

NEVER do these:
- Add filler phrases that weren't in the original ("of considerable value", "in large measure")
- Nominalize verbs ("recommendation of music" instead of "recommend music")
- Make sentences longer or more formal than the original
- Use passive voice where the original was active
- Add hedging clauses the original didn't have
- Sound more formal, academic, or verbose than the input

YOU MUST do these:
- Keep or shorten sentences — never bloat them
- Use the simplest word that fits ("start" not "commence", "use" not "utilize", "help" not "facilitate")
- Vary sentence LENGTH — mix short punchy sentences with longer ones
- Write the way a thoughtful person actually talks and writes
- Occasionally start a sentence with "And", "But", or "So" for natural flow
- Break up overly smooth, flowing text — real writers are sometimes abrupt
- Preserve ALL original meaning, facts, and structure exactly

Register: ${toneLabel}
Revision depth: ${intensityLabel}

Return ONLY this exact JSON shape, nothing else:
{
  "refined": "<the rewritten text as a single string>",
  "changes": [
    { "type": "<burstiness|vocabulary|perplexity|restructure|tone|transition>", "description": "<one sentence describing what was changed and why>" }
  ]
}`;

      const userPrompt = `Rewrite the following text to sound like a real human wrote it. Keep the meaning identical but make it feel natural, direct, and varied — not polished or formal.

BAD example (do NOT do this):
"Machine learning often commences with a subtle observation of patterns that may elude human notice."
GOOD example (do THIS):
"Machine learning usually starts by picking up on small patterns people miss."
...
${inputText.trim()}`;
      ...
      if (!response.ok) {
        ...
        throw new Error((err as {error?: {message?: string}})?.error?.message ?? `Groq API error: ${response.status}`);
      }
      ...
      const refined: string = parsed.refined ?? inputText.trim();
      const changes: ChangeNote[] = Array.isArray(parsed.changes) ? parsed.changes : [];
      if (changes.length === 0) {
        changes.push({ type: "vocabulary", description: "No significant mechanical patterns were detected. The text already reads naturally for the selected register and depth." });
      }
      ...
      setResult({
        ...
        humanized: refined,
        changes,
        wordCountOriginal: inputText.trim().split(/\s+/).length,
        wordCountHumanized: refined.split(/\s+/).length,
      });
      setActiveTab("output");
    } catch (e) {
      setError("Refinement failed: " + (e instanceof Error ? e.message : String(e)));
    } finally {
      setLoading(false);
    }
  }, [inputText, tone, intensity, wc, groqKey]);

  const handleClear = () => { setInputText(""); setResult(null); setError(""); };

  const tones: { value: ToneOption; label: string; desc: string }[] = [
    { value: "academic",       label: "Academic",       desc: "Scholarly register, no contractions" },
    { value: "professional",   label: "Professional",   desc: "Formal & authoritative"              },
    { value: "neutral",        label: "Neutral",        desc: "Clear & balanced"                    },
    { value: "conversational", label: "Conversational", desc: "Natural & readable"                  },
  ];

  const intensities: { value: IntensityOption; label: string; desc: string; active: string }[] = [
    { value: "subtle",     label: "Subtle",     desc: "Vocabulary only — minimal intervention",      active: "border-emerald-400 bg-emerald-50 text-emerald-800" },
    { value: "moderate",   label: "Moderate",   desc: "Vocabulary + structure — balanced revision",  active: "border-amber-400 bg-amber-50 text-amber-800"       },
    { value: "aggressive", label: "Thorough",   desc: "Full discourse-level refinement",             active: "border-red-400 bg-red-50 text-red-800"             },
  ];
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <header className="bg-white border-b border-slate-200 shadow-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Academic Writing Refinement Tool</h1>
            <p className="text-xs text-slate-500 mt-0.5">Improves naturalness, clarity, and stylistic authenticity of AI-assisted academic drafts</p>
          </div>
          <div className="hidden sm:flex gap-1.5">
            ...
            <span className="text-xs font-medium bg-emerald-600 text-white px-2.5 py-1 rounded-full">Writing Refinement</span>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl px-5 py-4">
          <p className="text-sm font-semibold text-blue-900 mb-1">About This Tool</p>
          <p className="text-xs text-blue-700 leading-relaxed">
            This tool improves the <strong>naturalness, clarity, and stylistic authenticity</strong> of AI-assisted drafts while preserving original meaning, author intent, and academic integrity.
            It does not claim to bypass or defeat AI-detection systems, and makes no guarantees regarding detector outcomes.
            AI detectors are probabilistic — high-quality academic writing may still be flagged. Use responsibly and transparently.
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
          ...
          <div className="relative">
            <input
              ...
              placeholder="gsk_••••••••••••••••••••••••••••••••••••••••••••••••••••••"
              className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent pr-20 font-mono"
            />
            <button onClick={() => setShowKey(v => !v)}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-400 hover:text-slate-600 font-medium px-2 py-1 rounded-lg hover:bg-slate-50 transition-colors">
              {showKey ? "Hide" : "Show"}
            </button>
          </div>
          <p className="text-[10px] text-slate-400 mt-1.5">Your key is used only in this browser session and never stored or sent anywhere except Groq's API.</p>
        </div>

        <div className="grid sm:grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
            <label className="block text-sm font-semibold text-slate-700 mb-3">Target Register</label>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
            <label className="block text-sm font-semibold text-slate-700 mb-3">Revision Depth</label>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-6 pt-5 pb-4">
            <label className="block text-sm font-semibold text-slate-700 mb-2">Paste your AI-assisted draft</label>
            <textarea
              ...
              placeholder={"Paste your AI-assisted text here… (minimum 20 words)\n\nThis tool will refine:\n• Mechanical or repetitive phrasing\n• Overly uniform sentence rhythm\n• Formulaic transitions and openers\n• Verbose constructions that reduce clarity"}
              value={inputText}
            ...
            <div className="flex items-center justify-between mt-2">
              <span className={`text-xs ${wc >= 20 ? "text-slate-400" : "text-amber-500 font-medium"}`}>
                {wc} word{wc !== 1 ? "s" : ""}
                {wc > 0 && wc < 20 ? ` · need ${20 - wc} more` : wc >= 20 ? " · ready to refine" : ""}
              </span>
              {error && <span className="text-xs text-red-500 font-medium">{error}</span>}
            </div>
          </div>
          <div className="bg-slate-50 border-t border-slate-100 px-6 py-4 flex items-center gap-3">
            <button onClick={handleHumanize} disabled={wc < 20 || loading}
              className="flex-1 sm:flex-none bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 disabled:cursor-not-allowed text-white font-semibold text-sm px-8 py-2.5 rounded-xl transition-colors shadow-sm flex items-center gap-2">
              {loading ? (
                <>
        ...
        {result && (
          <>
            <div className="grid grid-cols-3 gap-3">
              ...
              <StatChip label="Refined Words" value={String(result.wordCountHumanized)}
                sub={wcDiff > 0 ? `+${wcDiff} added` : wcDiff < 0 ? `${wcDiff} removed` : "same length"} />
              <StatChip label="Refinements Applied" value={String(result.changes.length)} sub="techniques used" />
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              {activeTab === "output" && (
                <div className="px-6 py-5">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Refined Text</span>
              ...
              {activeTab === "changes" && (
                <div className="px-6 py-5 space-y-2.5">
                  <p className="text-xs text-slate-400 mb-4">Writing refinement techniques applied to your text:</p>
            ...
            <div className="bg-slate-900 rounded-2xl px-5 py-4 flex items-center justify-between gap-4">
              <div>
                <p className="text-white text-sm font-semibold">Responsible Use Notice</p>
                <p className="text-slate-400 text-xs mt-0.5">AI detectors are probabilistic — no tool can guarantee detection outcomes. Always disclose AI assistance per your institution's academic integrity policy.</p>
              </div>
              <div className="flex-shrink-0 flex items-center gap-2 bg-slate-700 text-slate-300 font-semibold text-sm px-4 py-2.5 rounded-xl text-center leading-tight">
                <svg className="h-4 w-4 text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                </svg>
                <span className="text-xs">Academic<br/>Integrity</span>
              </div>
            </div>
          </>
        )}
        ...
        <div className="bg-slate-800 text-slate-300 rounded-2xl p-5 text-xs leading-relaxed grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          <div>
            <p className="font-semibold text-white mb-1.5">① Core Goal: Reduce Mechanical Phrasing</p>
            <p className="text-slate-400">Formulaic constructions such as "furthermore," "it is important to note," and "plays a pivotal role" are replaced with contextually equivalent alternatives. The goal is improved <strong className="text-slate-200">naturalness and stylistic authenticity</strong>, not the defeat of any detection system.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">② Sentence Rhythm &amp; Burstiness</p>
            <p className="text-slate-400">AI-generated prose tends toward uniform sentence length. This tool introduces natural variation — splitting long compound sentences at logical points — to better reflect the <strong className="text-slate-200">rhythmic variation characteristic of human academic writing</strong>.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">③ Passive-to-Active Voice</p>
            <p className="text-slate-400">Overly passive constructions (e.g., "It has been shown that") are rewritten to active equivalents. This improves <strong className="text-slate-200">clarity and authorial directness</strong> — a well-established standard in academic style guides including APA and Chicago.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">④ Beyond Synonym Swapping</p>
            <p className="text-slate-400">Simple synonym substitution is insufficient because AI patterns operate at the discourse level — repeated clause structures, predictable transitions, and uniform paragraph rhythms. This tool addresses <strong className="text-slate-200">sentence- and paragraph-level coherence</strong>, not just word choice.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">⑤ Verbose Construction Simplification</p>
            <p className="text-slate-400">Phrases such as "in order to," "due to the fact that," and "has the ability to" are condensed. This reduces nominal bloat and improves <strong className="text-slate-200">reading fluency</strong> — a core criterion in academic writing quality assessment.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">Limitations &amp; Safeguards</p>
            <p className="text-slate-400"><strong className="text-slate-200">All changes are purely stylistic.</strong> Factual content, citations, and argument structure are never altered. Note that AI detectors remain probabilistic — high-quality academic writing may still be flagged. Transparent disclosure of AI assistance is always recommended.</p>
          </div>
        </div>

        <div className="flex items-center justify-center gap-2 py-2">
          <span className="text-xs text-slate-400">Powered by</span>
          <span className="text-xs font-semibold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full">Groq · Llama 3.3 70B</span>
          <span className="text-xs text-slate-400">— free inference, no data retention</span>
        </div>
