...
function humanizeText(
  ...
  if (s1.count > 0) {
    changes.push({
      type: "vocabulary",
      description: `Replaced ${s1.count} AI-flagged word or phrase — e.g. "furthermore" → "also", "utilize" → "use", "pivotal" → "key". Original meaning preserved throughout.`,
    });
  }
  ...
  if (s2.count > 0) {
    changes.push({
      type: "transition",
      description: `Simplified ${s2.count} verbose construction${s2.count > 1 ? "s" : ""} — e.g. "in order to" → "to", "due to the fact that" → "because", "has the ability to" → "can". Meaning unchanged.`,
    });
  }
  ...
  if (s3.count > 0) {
    changes.push({
      type: "restructure",
      description: `Restructured ${s3.count} sentence${s3.count > 1 ? "s" : ""} — rewrote passive patterns to active voice, e.g. "It is widely recognized that" → "Most people agree that". Style-conditioned for ${tone} writing.`,
    });
  }
  ...
  if (s4.count > 0) {
    changes.push({
      type: "tone",
      description: `Inserted ${s4.count} contraction${s4.count > 1 ? "s" : ""} — "do not" → "don't", "cannot" → "can't", "it is" → "it's". Reduces the unnaturally formal register of AI writing.`,
    });
  }
  ...
  if (s5.splits > 0) {
    changes.push({
      type: "burstiness",
      description: `Split ${s5.splits} long sentence${s5.splits > 1 ? "s" : ""} at natural conjunction points — ", and", ", but", "; ". Creates the short/long variation that distinguishes human from AI rhythm.`,
    });
  }
  ...
  if (changes.length === 0) {
    changes.push({
      type: "vocabulary",
      description: "No strong AI patterns found. Try Aggressive intensity, or paste text with words like 'furthermore', 'utilize', 'pivotal', 'holistic', or phrases like 'in order to', 'it is important to note'.",
    });
  }
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 space-y-5">
        ...
        <div className="bg-slate-800 text-slate-300 rounded-2xl p-5 text-xs leading-relaxed grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          <div>
            <p className="font-semibold text-white mb-1.5">① Vocabulary Replacement</p>
            <p className="text-slate-400"><strong className="text-slate-200">80+ AI-flagged words replaced safely</strong> — "furthermore" → "also", "utilize" → "use", "pivotal" → "key", "in conclusion" → "overall". Every replacement preserves the original meaning exactly.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">② Verbose Construction Simplification</p>
            <p className="text-slate-400"><strong className="text-slate-200">Wordy AI constructions simplified</strong> — "in order to" → "to", "due to the fact that" → "because", "has the ability to" → "can". These are high-signal patterns that detectors flag strongly.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">③ Passive-to-Active Rewriting</p>
            <p className="text-slate-400"><strong className="text-slate-200">Passive AI openers rewritten</strong> — "It is widely recognized that" → "Most people agree that", "It has been shown that" → "Research shows that". Style-conditioned per tone.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">④ Contraction Insertion</p>
            <p className="text-slate-400"><strong className="text-slate-200">Two-word forms contracted</strong> — "do not" → "don't", "cannot" → "can't", "it is" → "it's". AI writing avoids contractions entirely. Academic tone skips this step.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">⑤ Burstiness — Sentence Splitting</p>
            <p className="text-slate-400"><strong className="text-slate-200">Long sentences split at natural points</strong> — ", and", ", but", "; ", ", which". Creates the short/long variation that human writing has naturally. No words are added — only a split point is moved.</p>
          </div>
          <div>
            <p className="font-semibold text-white mb-1.5">Meaning Always Preserved</p>
            <p className="text-slate-400"><strong className="text-slate-200">All changes are purely stylistic.</strong> The humanizer never adds ideas, removes content, or changes facts. Every transformation is a like-for-like substitution or structural simplification.</p>
          </div>
        </div>
