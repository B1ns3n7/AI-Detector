...
function humanizeText(
  ...
  // Shared tracker — prevents multiple modifications stacking on the same sentence
  ...
  // Step 7: Perplexity boost — hedge openers on unmodified sentences
  const s7 = boostPerplexity(s6.text, tone, intensity, modifiedIndices);
  if (s7.count > 0) {
    changes.push({
      type: "perplexity",
      description: `Added ${s7.count} hedging opener${s7.count > 1 ? "s" : ""} — "That said,", "In practice,", "Notably," — to introduce mild unpredictability without altering meaning.`,
    });
  }

  // Step 8: Readability variation (aggressive only) — natural qualifiers
  const s8 = addReadabilityVariation(s7.text, tone, intensity, modifiedIndices);
  if (s8.count > 0) {
    changes.push({
      type: "perplexity",
      description: `Inserted ${s8.count} natural qualifier${s8.count > 1 ? "s" : ""} — e.g. "in most cases", "for the most part". Human writers add these unconsciously; AI text characteristically omits them.`,
    });
  }
