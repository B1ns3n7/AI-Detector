...
function getTier(aiPct: number): { label: string; color: string; bg: string; border: string; ring: string; dot: string } {
  if (aiPct < 20)  return { label: "Likely Human",        color: "#16a34a", bg: "#f0fdf4", border: "#bbf7d0", ring: "#22c55e", dot: "bg-emerald-500" };
  if (aiPct < 35)  return { label: "Mostly Human",        color: "#65a30d", bg: "#f7fee7", border: "#d9f99d", ring: "#84cc16", dot: "bg-lime-500"    };
  if (aiPct < 55)  return { label: "Mixed / Uncertain",   color: "#d97706", bg: "#fffbeb", border: "#fde68a", ring: "#f59e0b", dot: "bg-amber-500"  };
  if (aiPct < 75)  return { label: "Likely AI",           color: "#ea580c", bg: "#fff7ed", border: "#fed7aa", ring: "#f97316", dot: "bg-orange-500" };
  return               { label: "Almost Certainly AI",  color: "#dc2626", bg: "#fef2f2", border: "#fecaca", ring: "#ef4444", dot: "bg-red-500"     };
}
...
export default function DetectorPage() {
  ...
  const getCombined = () => {
    ...
    const avgHuman = 100 - avgAI - avgMixed;
    return { avgAI, avgMixed, avgHuman, tier: getTier(avgAI) };
  };
  ...
  const getConsensusBanner = () => {
    ...
    if (pHigh && bHigh && nHigh) return { text: `All 3 engines: elevated patterns — review carefully`,          color: "#dc2626", bg: "#fef2f2", border: "#fecaca", icon: "▲" };
    if (pHigh && bHigh)          return { text: "Both heuristic engines elevated — neural pending/inconclusive", color: "#ea580c", bg: "#fff7ed", border: "#fed7aa", icon: "▲" };
  ...
  const handleAnalyze = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        const now = Date.now();
        if (_analysisCache && _analysisCache.text === trimmed && now - _analysisCache.timestamp < CACHE_TTL) {
        ...
        _analysisCache = { text: trimmed, perpResult: pFinal, burstResult: bFinal, timestamp: Date.now() };
    ...
    setTimeout(() => {
      runNeuralEngine(trimmed, engineAContextRef.current, engineBContextRef.current)
            ...
            {(perpResult || burstResult) && (
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="mx-6 mb-5 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3">
                  ...
                  <p className="text-xs text-amber-700 leading-relaxed">Results are probabilistic pattern analysis only. Formal academic writing, ESL writing, and extensively revised human text may share surface patterns with AI-generated text. No automated decision should be based on these results alone. Always apply professional judgment.</p>
                </div>
              </div>
            )}
      ...
      <footer className="mt-12 border-t border-slate-200 bg-white">
        <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p className="text-[11px] text-slate-400 text-center">
            Results are probabilistic. 3-engine consensus. 100+ signals. GPTZero &amp; Turnitin aligned thresholds.
          </p>
